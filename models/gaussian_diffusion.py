...
class GaussianDiffusion:
    ...
    def training_losses(
        ...
        :param y: the [N x C x ...] tensor of degraded inputs. 其实就是加了噪声或者进行了上采样的数据
        :param t: a batch of timestep indices.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param noise: if specified, the specific Gaussian noise to try to remove.
        :return: a dict with the key "loss" containing a tensor of shape [N].
                 Some mean or variance settings may also have other keys. 也就是关于N个样本的损失函数的构建
        """
        if model_kwargs is None:
            model_kwargs = {}
        
        # inital state setting
        z_y = self.encode_first_stage(y, first_stage_model, up_sample=False)
